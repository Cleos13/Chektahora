...
$id = $data->data->object->metadata->Id;
$type = $data->type;
$link = '<a href=\"https://admin.conekta.com/orders/'.$data->data->object->id.'\" class="link">Pago aprobado. (Conekta)</a>';
if ($type == 'charge.paid' || $type == 'order.paid'){
    date_default_timezone_set("America/Mexico_City");
    $hora = date('d-m-Y h:i A');
    $conn->query("UPDATE its SET status_customer = 'Pago recibido, verificar en panel (opcional)', status = 1, notify_pay = '$link', hora_status_customer='$hora' WHERE ID_compra ='$id'");
    http_response_code(200);
    $email = $data->data->object->customer_info->email;
    $products = $data->data->object->line_items->data;
    $total = $data->data->object->amount / 100;
    $num = count($products);
    $num_p = count($p);
    $template = '';
    for($i = 0; $i < $num; $i++){
    ...
    http_response_code(200);
    echo 'Notificacion recibida en webhook';
}
?>