<div class="product-container large-12 medium-12 small-12 grid-padding-x text-center align-self-middle spaceSection">
  <div class="product-item">
    <h2 style="font-weight: bold; color: #662480">Prueba PCR para detección de influenza + COVID</h2>
    <img src="../img/Producto-Covid-19-Anticuerpos-Sitio-Web.jpg" alt="Producto 1" class="llamativo" width="450" height="480">
    <p style="font-weight: bold; font-size: 28px; color:black;" class="price"> $1,399</p>
    <div class="buttons">
    <a  style="  border: 2px solid purple; " href="formulario5.php">
        <img style="height: 20px; width: 130px;" src="../css/img/logitooo.png" alt="Texto alternativo de la imagen">
    </a>
    <a  style="  border: 2px solid purple; " href="https://buy.stripe.com/6oE02le1I3zmaPu6s2">
        <img style="height: 20px; width: 130px;" src="../css/img/stripe2.png" alt="Texto alternativo de la imagen">
    </a>
    </div>
    <div class="buttons">
      <a href="#" class="more-button">Ver más</a>
    </div>
    <div class="additional-info d-none">
      <h3 style="font-size: 20px; color: #4F2D83;">
        Prueba PCR de COVID-19 + Influenza.
      </h3>
      <ul>
        <li>Tipo de muestra: Nasofaríngea (nasal) y orofaríngea </li>(oral).
        <li>Toma de muestra realizada por enfermeros capacitados.</li>
        <li>Tiempo de entrega: 24 horas hábiles.</li>
      </ul>
      <p>
        Indicada a partir del segundo día después de la posible exposición al virus.
        Única prueba avalada para emitir un diagnóstico de COVID-19 e influenza.
      </p>
      <p>
        Nuestro diagnóstico por PCR abarca el SARS-CoV-2, el virus de influenza tipo A, el virus Sincicial Respiratorio en Humano tipo A y el virus Sincicial Respiratorio en Humano tipo B.
      </p>
    </div>
  </div>

  <div class="product-item">
    <h2 style="font-weight: bold; color: #662480">Prueba PCR para detección de COVID</h2>
    <img src="../img/Producto-Covid-19-PCR-Sitio-Web.jpg" alt="Producto 2" class="llamativo" width="450" height="480">
    <p style="font-weight: bold; font-size: 28px; color:red;" class="price"><del style="color: black;">$1,100</del> $599</p>
    <div class="buttons">
    <a  style="  border: 2px solid purple; " href="formulario6.php">
        <img style="height: 20px; width: 130px;" src="../css/img/logitooo.png" alt="Texto alternativo de la imagen">
    </a>
    <a  style="  border: 2px solid purple; " href="https://buy.stripe.com/6oEbL38Hoc5S3n23fS">
        <img style="height: 20px; width: 130px;" src="../css/img/stripe2.png" alt="Texto alternativo de la imagen">
    </a>
    </div>
    <div class="buttons">
      <a href="#" class="more-button">Ver más</a>
    </div>
    <div class="additional-info d-none">
      <h3 style="font-size: 20px; color: #4F2D83;">
        Prueba PCR de COVID-19.
      </h3>
      <ul>
        <li>Tipo de muestra: Nasofaríngea (nasal) y orofaríngea </li>(oral).
        <li>Toma de muestra realizada por enfermeros capacitados.</li>
        <li>Tiempo de entrega: 24 horas hábiles.</li>
      </ul>
      <p>
        Indicada a partir del segundo día después de la posible exposición al virus.
        Única prueba avalada para emitir un diagnóstico de COVID-19.
      </p>
    </div>
  </div>

  <div class="product-item">
    <h2 style="font-weight: bold; color: #662480">Prueba de Antígenos para detección de COVID</h2>
    <img src="../img/Producto-Covid-19-Influenza-Sitio-Web.jpg" alt="Producto 3" class="llamativo" width="450" height="480">
    <p style="font-weight: bold; font-size: 28px; color:black;" class="price"> $599</p>
    <div class="buttons">
    <a  style="  border: 2px solid purple; " href="formulario7.php">
        <img style="height: 20px; width: 130px;" src="../css/img/logitooo.png" alt="Texto alternativo de la imagen">
    </a>
    <a  style="  border: 2px solid purple; " href="https://buy.stripe.com/fZe5mFbTA0nae1GcQp">
        <img style="height: 20px; width: 130px;" src="../css/img/stripe2.png" alt="Texto alternativo de la imagen">
    </a>
    </div>
    <div class="buttons">
      <a href="#" class="more-button">Ver más</a>
    </div>
    <div class="additional-info d-none">
      <h3 style="font-size: 20px; color: #4F2D83;">
        Prueba de Antígenos de COVID-19, registradas y certificación de calidad TUV y GMP, con una sensibilidad y especificidad mayor al 90%.
      </h3>
      <ul>
        <li>Tipo de muestra: Nasofaríngea (nasal).</li>
        <li>Toma de muestra realizada por enfermeros capacitados.</li>
        <li>Tiempo de entrega: 20 minutos de forma verbal y 6 horas hábiles por escrito.</li>
      </ul>
      <p>
        Indicada a partir del cuarto día después de la posible exposición al virus.
        En caso de resultar positivo, se debe complementar con una prueba confirmatoria a través de PCR.

      </p>
    </div>
  </div>

  <div class="product-item">
    <h2 style="font-weight: bold; color: #662480">Prueba de Anticuerpos para detección de COVID</h2>
    <img src="../img/Producto-Covid-19-PCR-Sitio-Web.jpg" alt="Producto 4" class="llamativo" width="450" height="480">
    <p style="font-weight: bold; font-size: 28px; color:black;" class="price">$1,050</p>
    <div class="buttons">
    <a  style="  border: 2px solid purple; " href="formulario8.php">
        <img style="height: 20px; width: 130px;" src="../css/img/logitooo.png" alt="Texto alternativo de la imagen">
    </a>
    <a  style="  border: 2px solid purple; " href="https://buy.stripe.com/cN28yR0aSee05va4jI">
        <img style="height: 20px; width: 130px;" src="../css/img/stripe2.png" alt="Texto alternativo de la imagen">
    </a>
    </div>
    <div class="buttons">
      <a href="#" class="more-button">Ver más</a>
    </div>
    <div class="additional-info d-none">
      <h3 style="font-size: 20px; color: #4F2D83;">
        Prueba de Anticuerpos COVID-19.
      </h3>
      <ul>
        <li>Tipo de muestra: Sanguínea.</li>
        <li>Toma de muestra realizada por enfermeros capacitados.</li>
        <li>Tiempo de entrega: 24 horas hábiles.</li>
      </ul>
      <p>Diagnóstico para Anticuerpos IgM e IgG para COVID-19. </p>
    </div>
  </div>

</div>